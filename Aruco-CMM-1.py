...
camera_id = 1               #1 = external cam, 0 = webcam
...
static_img_path = "F:\\Vs Code\\images\\CMM_Machine_Psuedo.jpeg"     #PUT IMAGE HERE IF NEEDED LATER
...
loc = "F:\\Vs Code\\CMM_Major_Project\\roiconfig.xls"
print(loc)
...
if camera_id == -1:
  resolution_x = 1280 #4272   #1920
  resolution_y = 720 #2848   #1080
  ...
  if resolution_setting == 1:
    print("selected res: 1280x720")
    cap.set(cv2.CAP_PROP_FRAME_WIDTH, 1280)
    cap.set(cv2.CAP_PROP_FRAME_HEIGHT, 720)
    known_focal_length_1080Q  = 1150#1115  #1127    #1115 #1115 at 1920x1080 #490
...
center_x = int(resolution_x / 2)
center_y = int(resolution_y / 2)

# Define the size of the ROI (width and height)
roi_width = 2 * (centre_square_side / 2)  # Use your desired width
roi_height = 2 * (centre_square_side / 2)  # Use your desired height

# Calculate the coordinates for the ROI
aruco_x1 = center_x - int(roi_width / 2)
aruco_x2 = center_x + int(roi_width / 2)
aruco_y1 = center_y - int(roi_height / 2)
aruco_y2 = center_y + int(roi_height / 2)

# Extract the centered ROI
...
while True:
    ...
    # Define a custom window name
    window_name = 'My Custom Window'
    # Create a named window with the custom name
    cv2.namedWindow(window_name, cv2.WINDOW_NORMAL)
    # Set the size of the window (width, height)
    custom_window_size = (800, 600)  # You can adjust the size as needed
    cv2.resizeWindow(window_name, custom_window_size[0], custom_window_size[1])
    cv2.imshow(window_name, img)  # Display the 'img' in the custom window
